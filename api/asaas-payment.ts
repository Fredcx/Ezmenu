...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        const { order_id, amount, customer, establishment_id } = req.body;

        if (amount < 5) {
            return res.status(400).json({ error: "O valor mínimo para pagamentos via Pix na Asaas é R$ 5,00 (Exigência do Banco Central/Asaas)." });
        }
