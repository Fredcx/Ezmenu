...
export const BrandingLogo: React.FC<BrandingLogoProps> = ({
    ...
    const customLogo = !skipCustomization && establishment?.settings?.header_image;
    const customName = !skipCustomization && establishment?.name;
    ...
    const [imgSrc, setImgSrc] = useState(supabaseUrl);
    ...
    const content = (
        <>
            <div className={`rounded-xl bg-transparent flex items-center justify-center relative overflow-hidden ${layout === 'horizontal' ? 'w-10 h-10' : 'w-full h-full'}`}>
                <img
                    src={customLogo || imgSrc}
                    alt={customName || "EZ Menu Logo"}
