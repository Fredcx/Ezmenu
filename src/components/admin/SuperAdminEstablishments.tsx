...
export const SuperAdminEstablishments = () => {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (funcError || (funcData && funcData.error)) {
                ...
                setNewRest({ name: '', slug: '', adminName: '', adminEmail: '', adminPassword: '', email: '' });
                fetchEstablishments();
            }
    ...
    const deleteEstablishment = async (id: string, name: string) => {
        if (!confirm(`Tem certeza que deseja excluir o restaurante "${name}"? Esta ação apagará permanentemente todos os dados (pedidos, clientes, etc) vinculados a ele.`)) return;
        ...
        try {
            ...
            if (error) {
                ...
                if (error.code === '23503') { // Foreign key violation
                    toast.error("Erro de Conflito (409)", {
                        description: "Existem pedidos ou dados vinculados. Aplique o script de correção SQL no Supabase para permitir a exclusão em cascata."
                    });
                } else {
                    throw error;
                }
