...
import { LayoutDashboard, UtensilsCrossed, Armchair, LogOut, Menu, Bell, CreditCard, Package, ChefHat, TrendingDown, TrendingUp, Globe, Users, Calendar, Settings, Store, LayoutGrid, ShoppingBag, Eye } from 'lucide-react';
            ...
            try {
                ...
                data?.forEach((req: any) => {
                    if (!notifiedIdsRef.current.has(req.id)) {
                        ...
                        toast.custom((t) => (
                            <div className="bg-card/95 backdrop-blur-xl border border-red-500/30 rounded-2xl p-4 shadow-2xl flex items-center gap-4 min-w-[320px] animate-in slide-in-from-right-10 cursor-pointer hover:border-red-500 transition-colors relative"
                                onClick={() => targetPath && navigate(targetPath)}>
                                <button onClick={(e) => { e.stopPropagation(); toast.dismiss(t); }} className="absolute top-2 right-2 p-2 -mt-1 -mr-1 text-muted-foreground hover:text-red-500 transition-colors">✕</button>
        ...
        <div className="flex h-screen bg-secondary/10">
            ...
            <aside className="hidden md:flex w-72 flex-col bg-white border-r border-border/40 h-full shadow-premium relative z-50">
                <div className="p-8 border-b border-border/20 flex items-center gap-3">
                    <div className="flex flex-col">
                        <span className="font-black text-xl uppercase tracking-tighter italic leading-none text-foreground">
                            {slug?.replace('-', ' ').toUpperCase()}
                        </span>
                    </div>
                    <div className="h-4 w-px bg-border/60 mx-1 rotate-[20deg]" />
                    <BrandingLogo variant="dark" className="w-10 h-10" showText={false} skipCustomization={true} />
                </div>
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                <div className={`fixed inset-0 z-50 transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                    ...
                    {/* Slide-in Sidebar */}
                    <div className={`absolute left-0 top-0 bottom-0 w-[280px] bg-white shadow-2xl transition-transform duration-300 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col p-6`}>
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-2">
                                <span className="font-black text-lg uppercase tracking-tighter italic leading-none text-foreground">
                                    {slug?.replace('-', ' ').toUpperCase()}
                                </span>
                                <div className="h-3 w-px bg-border/60 mx-1 rotate-[20deg]" />
                                <BrandingLogo variant="dark" className="w-8 h-8" showText={false} skipCustomization={true} />
                            </div>
                            <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 hover:bg-secondary rounded-xl transition-colors">
                        ...
                        <nav className="flex-1 space-y-1.5 overflow-y-auto no-scrollbar">
                            {navItems.map((item) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3.5 px-5 py-4 rounded-2xl transition-premium group ${location.pathname === item.path
                                        ? 'bg-primary text-white shadow-lg shadow-primary/20'
                                        : 'hover:bg-secondary/50 text-muted-foreground hover:text-foreground'
                                        }`}
                                >
                        ...
                        <div className="mt-8 pt-6 border-t border-border/20">
                            <button
                                onClick={handleLogout}
                                className="w-full flex items-center gap-3.5 px-5 py-4 rounded-2xl text-red-500 bg-red-50 font-black text-[13px] uppercase tracking-tight active:scale-95"
                            >
