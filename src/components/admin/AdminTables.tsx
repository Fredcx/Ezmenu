...
export function AdminTables() {
    ...
    const releaseTable = async (tableId: string) => {
        ...
        try {
            // 1. Get all session orders for this table
            ...
            if (tableOrders && tableOrders.length > 0) {
                ...
                // 2. Archive all orders for this table
                await supabase
                    .from('orders')
                    .update({
                        status: 'archived',
                ...
                // 3. Mark items as ready if they weren't
                await supabase
                    .from('order_items')
                    .update({ status: 'ready' })
            ...
            // 3.5 Clear service requests history
            if (establishmentId) {
                await supabase
        ...
        tableOrders.forEach((order: any) => {
            if (order.order_items) {
                const itemsWithMeta = order.order_items.map((oi: any) => ({
                    ...
                    isRodizio: oi.menu_items?.is_rodizio, // This comes from menu, but we'll use price for clarity
                    _orderAuthor: order.customer_name || 'Cliente', 
    ...
    return (
        <div className="space-y-6 h-full flex flex-col animate-in fade-in duration-500" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
            ...
            <Dialog open={!!selectedTable} onOpenChange={() => setSelectedTable(null)}>
                <DialogContent className="max-w-3xl w-[95vw] sm:w-[90vw] md:w-full bg-white/70 backdrop-blur-2xl border border-white/40 shadow-2xl p-0 gap-0 overflow-hidden rounded-[2rem]">
                    ...
                    {['occupied', 'ordered', 'eating', 'timeout', 'waiting_payment'].includes(getTableStatus(selectedTable || '')) ? (
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                {getTableParticipants().length > 0 ? getTableParticipants().map((p, idx) => (
                                                    <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-secondary/30 border border-border/50">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center text-primary font-bold text-xs ring-2 ring-background uppercase">
                                                                {p.name.substring(0, 2)}
                                                            </div>
                                                            <div className="flex flex-col">
                                                                <span className="font-bold text-sm leading-none mb-0.5">{p.name}</span>
                                                                <span className="text-[10px] text-muted-foreground font-medium truncate max-w-[120px]">{p.email}</span>
                                                            </div>
                                                        </div>
                                                        {p.type === 'rodizio' ? (
                                                        ...
                                                        <div key={idx} className="group flex items-center gap-4 p-4 rounded-2xl bg-card border border-border/40 hover:border-primary/20 hover:shadow-md transition-all duration-200 relative overflow-hidden">
                                                            ...
                                                            <div className="flex-1 z-10 min-w-0">
                                                                <div className="flex justify-between items-start">
                                                                    ...
                                                                    {/* Price OR Timer */}
                                                                    {(item.price === 0 && !item.code.startsWith('SYS')) ? (
                                                                ...
                                                                <div className="flex items-center gap-3 mt-1.5">
                                                                    {/* Type Badge */}
                                                                    {item.code.startsWith('SYS') ? (
                                                                        <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100 flex items-center gap-1 shrink-0">
                                                                            <Fish className="w-3 h-3" /> Rodízio (Adesão)
                                                                        </span>
                                                                    ) : item.price === 0 ? (
                                                                        <span className="text-[10px] font-bold uppercase tracking-wider text-orange-500 flex items-center gap-1 shrink-0">
                                                                            <UtensilsCrossed className="w-3 h-3" /> Rodízio (Incluso)
                                                                        </span>
                                                                    ) : (
                        ...
                        <div className="p-10 flex flex-col items-center justify-center space-y-8 bg-card animate-in fade-in duration-500">
                            ...
                            >
                                <QRCodeSVG
                                    value={`https://www.ezmenuu.com/${slug}?q=${selectedTable ? (selectedTable.length >= 2 ? `${selectedTable[0]}_${btoa('ezmenu_tbl_' + selectedTable)}_${selectedTable[selectedTable.length - 1]}` : btoa('ezmenu_tbl_' + selectedTable)) : ''}`}
                                    size={220}
                                    level="H"
                                    includeMargin={false}
                                />
