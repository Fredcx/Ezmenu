...
export function RodizioSelectionScreen({ onStartOrder, onBack }: HomeScreenProps) {
  const { setSession, session, currentClientId, clearCart, addDirectlyToSentOrders, resetTableOrders } = useOrder();
  const { t } = useLanguage();
  ...
  const handleConfirm = () => {
    ...
    if (adultsCount > 0) {
      const adultItem = {
        ...
        price: additionalCharges[0].price,
        isRodizio: false,
    ...
    if (childrenCount > 0) {
      const childItem = {
        ...
        price: additionalCharges[1].price,
        isRodizio: false,
  ...
  return (
    <div className="h-full flex flex-col bg-background text-foreground relative overflow-hidden">
      ...
      {/* Hero Header */}
      <div className="pt-24 pb-4 text-center z-10 relative flex flex-col justify-end min-h-[100px]">
      ...
      {/* Main Content (Cards) */}
      <div className="flex-1 px-6 flex flex-col items-center justify-start pb-40 z-10 overflow-y-auto no-scrollbar">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 z-50 bg-gradient-to-t from-background via-background/95 to-transparent pt-12 pb-8">
