...
import { categories as localRodizioCats, alacarteCategories as localAlacarteCats, menuItems as localMenuItems } from '@/data/menuData';
...
export function MenuProvider({ children }: { children: ReactNode }) {
    ...
    const loadMenuData = async (establishmentId: string) => {
        ...
        try {
            ...
            if (catData) {
                ...
                catMap = catData;
                const validCatData = catData.filter(c => !c.name?.toUpperCase().includes('SISTEMA'));
                setCategories(validCatData.filter(c => c.type === 'rodizio').map(mapToFrontendCat));
                setAlacarteCategories(validCatData.filter(c => c.type === 'alacarte').map(mapToFrontendCat));
            }
            ...
            if (itemError) {
                ...
                // Filter system items from local menu data
                const systemItems = localMenuItems.filter(i => i.category === 'system');
                
                // Merge DB items with system items (avoiding duplicates if they eventually exist in DB)
                const finalItems = [...formattedItems];
                systemItems.forEach(sysItem => {
                    if (!finalItems.some(i => i.code === sysItem.code)) {
                        finalItems.push(sysItem);
                    }
                });
            ...
            if (user) {
                ...
                if (profile?.establishment_id) {
                    ...
                    if (estData) {
                        ...
                        setCurrentEstablishmentId(estData.id);
                        loadMenuData(estData.id);
                    }
                } else {
    ...
    useEffect(() => {
        ...
        const estChannel = supabase.channel('establishments_changes')
